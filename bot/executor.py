...
# 7 дней: за 6ч дрейф медленного аккруера (avKAT ~+7%/год = +0.005%/6ч) неотличим от нуля,
# а шум котируемых оракулов аннуализируется в сотни %/год — первое же окно 30.07 дало залп
# из 5 ложных «таймеров» против честного 7-дневного замера, где все рынки самозалечиваются.
RATE_WATCH_WINDOW_SEC = float(os.environ.get("KT_RATE_WATCH_WINDOW_SEC", "604800"))  # 7д
_YEAR = 31_536_000.0


def _rate_watch(r: dict, st: dict, now_ts: float) -> None:
    """Сравнить ставку займа с дрейфом оракула по рынкам, где у нас есть приз, и записать одну
    строку В ЛОГ на СМЕНУ знака (а не на каждый проход). Никогда не роняет проход.
    В TG не ходит: человек по смене режима ничего не делает — цели и так в hot-наборе, бот
    отработает пересечение сам (правило 30.07: алерт там, где нужен человек; «боты-опционы»)."""
    ...
    for mid, pz in prize.items():
        ...
        if flipped:
            print(f"  ⏱ {_mkt_name(mid)}: ставка займа {borrow_apr:.2f}%/год ОБОГНАЛА дрейф "
                  f"коллатерала {drift_apr:+.2f}%/год — цели на ${pz:,.0f} пошли вниз "
                  f"по расписанию, а не по случаю")
        else:
            print(f"  ⏱ {_mkt_name(mid)}: дрейф {drift_apr:+.2f}%/год снова обгоняет ставку "
